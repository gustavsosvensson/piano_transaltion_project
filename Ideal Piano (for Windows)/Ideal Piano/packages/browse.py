    ...
    def fileDialog(self, file_name=None):
        ...
        if file_name:
            ...
            self.filename = Dialog(
                ...
                directory=last_path,
                filter='MIDI files (*.mid);; PDF files (*.pdf);; all files (*)').filename[0]

        if '.pdf' in self.filename or '.PDF' in self.filename:

            self.msg_label.setText('Converting PDF to MIDI, please wait...')

            self.progress_bar = QtWidgets.QProgressBar(self.labelFrame)
            self.progress_bar.setGeometry(100, 200, 200, 30)  # Adjust the size and position as needed
            self.progress_bar.setMaximum(100)
            self.progress_bar.setValue(0)  # Start with 0%
            self.progress_bar.hide()

            QApplication.processEvents()

            # Construct the command to run the main.py script with the selected PDF file
            command = [sys.executable, "C:/dev/Piano visual/pdf2png_library/main.py", self.filename]
            process = Popen(command, stdout=PIPE, stderr=PIPE, text=True)

            self.progress_bar.show()  # Show the progress bar at the start of the conversion
            progress_update_count = 0

            with open('shared_data.json', 'r') as file:
                total_num = json.load(file)

            precentage = 100 / (total_num * 3 + 5)
            ...
            while True:
                ...
                if output:
                    ...
                    new_progress_value = min(progress_update_count * precentage, 100)  # Ensure it doesn't go over 100%
                    self.progress_bar.setValue(new_progress_value)
                    self.msg_label.setText(output.strip())
                    QApplication.processEvents()
            ...
            if process.returncode != 0:
                ...
                self.msg_label.setText('Conversion completed successfully.')
                base_name = os.path.splitext(os.path.basename(self.filename))[0]
                generated_mid_filename = f"{base_name}.mid"
                mid_file_path = os.path.join(f"C:\dev\Piano visual\Ideal Piano (for Windows)\Ideal Piano\data\merged_midi_output", generated_mid_filename)
                self.filename = mid_file_path
